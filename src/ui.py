# tictactoe_robot_ui_pyside6.py
"""
PySide6 GUI for Tic‑Tac‑Toe + robot arm (thread‑safe, non‑blocking).

🔄 **Новые фичи**
1. При калибровке кнопка **Play** становится неактивной и активируется лишь
   после завершения процесса.
2. Нажимая **Play** пользователь выбирает, кем играть: «X», «O» или «Random».
   Логика хода робота подстраивается под выбор.

Замени мок‑методы `grab_frame`, `_do_calibration`, `_do_move` реальными
вызовами SDK/OpenCV — UI останется отзывчивым.
"""
